...
void StepperDriver4PWM::setPwm(float Ualpha, float Ubeta, float Uc) {
  ...
  #ifdef STEPPER_CENTER_MODULATION
    float dc1_2 = _constrain(Ualpha/voltage_power_supply,-1.0f,1.0f) / 2;
    float dc2_2 = _constrain(Ubeta/voltage_power_supply,-1.0f,1.0f) / 2;

    duty_cycle1B = 0.5 + dc1_2;
    duty_cycle1A = 0.5 - dc1_2;
    
    duty_cycle2B = 0.5 + dc2_2;
    duty_cycle2A = 0.5 - dc2_2;
  #else
    if( Ualpha > 0 )
